...
module.exports = {
  ...
  // T2 (lib/process-control.js) переиспользует containment-примитив для проверки
  // "заглушка/каталог unit-файлов внутри test root" — экспортируем, чтобы process-control
  // не заводил вторую реализацию containment (бриф iso-t2 прямо это запрещает).
  contained,
};
