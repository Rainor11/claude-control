...
# возвращается ровно переданный литерал. Уровень «полностью скомпрометированный воркер того
# же uid» этим не закрывается и никогда не закрывался (см. честную границу в шапке
# bin/claude-auto-send).
#
# КОНТРАКТ ВЫЗОВА (K1, ревью T8) — ОБЯЗАТЕЛЕН, иначе шов сам становится дырой. Вызывающий
# обязан (а) присвоить боевой литерал ОБЫЧНЫМ присваиванием и (б) звать эту функцию ТОЛЬКО
# внутри `if [[ -n "${CLAUDE_CONTROL_TEST_ROOT+set}" ]]`:
#
...
#   fi
#
# Почему НЕ «безусловный вызов, а гейт внутри функции»: чтобы функция вообще существовала,
# вызывающий обязан её заsource'ить, а чтобы найти файл библиотеки — выполнить `readlink`/
# `dirname`/`cd`/`pwd`. Bash при старте импортирует функции из окружения (`BASH_FUNC_x%%=…`)
# и исполняет `$BASH_ENV` — подменённый `readlink` уводит резолв каталога в чужой, оттуда
# подключается ЧУЖАЯ process_control_notifier_path, и «неперекрываемый» путь становится
# перекрываемым. Ровно это и произошло в первой редакции T8 (воспроизведено на копиях: без
# маркера все четыре bash-точки отдавали /tmp/EVIL/telegram_notify.sh, до T8 — литерал).
# `command readlink` / `builtin cd` НЕ помогают: `command` и `pwd` перекрываются функцией
# так же. Снос импортированных функций первой строкой (`unset -f` по `declare -F`) тоже не
# спасает — `declare`, `unset` и `read` перекрываются функцией ровно так же (проверено), а
# в бою это ещё и меняет поведение. Единственный неподделываемый примитив — `[[ ]]`:
# зарезервированное слово, bash отказывается импортировать функцию с таким именем; `[` —
# обычный builtin, и `BASH_FUNC_[%%=() { return 0; }` открыл бы гейт.
#
# Python-точка (bot/rnr_workers_bot.py) держит ТУ ЖЕ форму штатно: литерал возвращается до
# любого обращения к bash-окружению.
#
