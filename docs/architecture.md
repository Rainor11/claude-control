...
### `lib/notify-card.sh` / `.js`

Единый рендер операторских уведомлений («карточка»): заголовок со статусным эмодзи, разделитель, блок фактов, именованные секции, цитата и блок «Что делать». Секции с пустым содержимым не печатаются вовсе - карточка короткая, когда всё в порядке, и разворачивается, когда есть проблема. До этой библиотеки каждая из ~30 точек отправки писала свою строку руками, и сводки приходили одной фразой с точками с запятой, где `🔴 не разослано - нет` занимало столько же места, сколько реальный отказ.

Библиотека владеет только рендером, экранированием, отбрасыванием пустых секций и бюджетом длины (3500 UTF-16 code units - те же единицы, которыми меряет Telegram; при переполнении вытесняются списки, затем цитата, затем факты, а заголовок и «Что делать» выживают всегда). Путь нотификатора, chat-id, `--reply-markup`, разбор `message_id` и стейт доставки остаются у вызывающего: `nc_send` / `send()` принимают уже отрезолвленный путь через `$NC_TG` / `opts.tg` и сами его не ищут. Из-за этого `bin/claude-auto-{tg,ask,send,run}` библиотеку не подключают вовсе - они держат путь нотификатора неперекрываемым (шов T8, см. [docs/testing.md](./testing.md)), а любой резолв каталога до присваивания этого пути открывал бы ровно ту дыру, которую шов закрыл; там карточки собраны plain-текстом инлайном.

Отправка идёт через `@RnR_Workers` (у кастомного бота `telegram_notify.sh` не добавляет шапку `🖥️ Сервер: …`); сломанный или нечитаемый токен - фолбэк на дефолтного бота, а не молчание. На ошибку разметки (`can't parse entities` / `error_code 400`) выполняется ровно один повтор plain-текстом со снятыми тегами: у `telegram_notify.sh` фолбэк только транспортный, а булев результат доставки несущий - на нём висит дедуп эпизодов в `claude-auto-liveness` и `dept-dispatcher`. Транспортные сбои не ретраятся (их отрабатывает сам нотификатор). Оба рендера, bash и node, обязаны давать побайтово одинаковую карточку - это проверяется в `tests/notify-card.test.sh`.
