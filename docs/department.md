...
успех: exit 1 → раннер пишет `exec_failed` и шлёт 🔴-карточку «Заявка НЕ
исполнена» (не молчаливая сводка с ошибкой внутри текста); повтор идемпотентен
...
TG-алерты исхода — карточки «✅ Заявка исполнена» / «🔴 Заявка НЕ исполнена» /
«🚨 Исполнено, но финал не записан», с полями заявки из ledger (описание,
автор, исполнитель, event_id) — читаются без похода в журнал. Рендер общий,
`lib/notify-card.sh` (см. [docs/architecture.md](./architecture.md)).
