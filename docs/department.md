...
15000, читается заново при каждом вызове). Наружу — через Angie-vhost с
basic auth (детали проброса/сертификата — server_docs). Весь HTML — в
...
- `GET /` — «Офис»: сводная строка (правила/открытые аппрувы/инциденты/
  ...
  архивариус, тп}: аватар, статус-бейджик 🔴/⏳/😴/⛔/🟢, mission_version,
...
- `GET /activity` — «бюджеты» v1 (активность-прокси, НЕ токены): события/
  компакции/wake·rebase за 7 дней + sparkline по дням за 14 дней.
