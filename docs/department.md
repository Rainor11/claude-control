...
см. «Связанное». Актуально на 2026-07-16 (фаза 3).
...
kb_change_request, decision_request`; `message_status.status` ∈ `acked,
handled, dead`; `approval_status.status` ∈ `approved, denied, executing,
executed, exec_failed`. `executing` — промежуточный статус исполнения
заявки (диспетчер помечает ДО запуска раннера, дедуп повторного запуска);
`executed`/`exec_failed` — финал, который пишет `dept-exec-runner`.
`executing`/`executed`/`exec_failed` блокируют `approval-resolve` («менять
решение поздно»).
...
руководителя»); из них диспетчер исполняет ТОЛЬКО четыре последних.
...
- `dept-mission-request --worker <w> --reason '<s>'` — смена миссии
  ...
  (≤16000 симв.).
- `dept-planerka-request --reason '<s>'` — плановый rebase всего отдела
  (kind_of `planerka`).
...
`dept-planerka-exec` — повторный rebase свежих сессий безвреден.

**Планёрка** (`dept-planerka-exec`): rebase всех воркеров отдела
(руководитель/мк/архивариус/тп) в state `active`. Busy-воркер — до 3
попыток × `PLANERKA_RETRY_SLEEP` (10с), затем в «отложены» (добьёт
rebase-check по возрасту / следующая планёрка / policy-турникет);
спящие пропускаются (не будятся); hard-error (rc∉{0,3}, напр.
STALE-память) — отдельный 🔴-сегмент. Сводка (stdout + TG) — 4 сегмента:
ребейзнуты / отложены (busy) / 🔴 ошибки / спят.

**role_change** (16.07): карточки ролей `отдел/роли/*.md` — 4-я
писательская база Архивариуса, правки СТРОГО через `dept-approve
--kind-of role_change` (дифф на карточке → ✅ оператора → применяет САМ
Архивариус). Диспетчер `role_change` намеренно НЕ исполняет (нет в
`EXEC_KINDS`) — гейт процедурный, уровень enforcement тот же, что у
`kb_change`. Состав ролей, структура отдела, спека — Оракул (запросы —
оператору).
...
2. **Исполнение утверждённых заявок**: `list --kind approval --status
   ...
   руководитель/operator) → пометить `executing` (дедуп в ledger: второй
...
исполнителей (basename ∈ dept-{spawn,mission,planerka,sleep}-exec И
каталог = каталог раннера), исполняет под `timeout` (числовая защита
...
**Без `DEPT_REBASE_ENFORCE=1` (текущий режим, обкатка) — только
`[would-rebase]`-алерт**, дедуп на эпизод (`rebase-check-state.json`;
эпизод сбрасывается при уходе воркера из active). С флагом — реально
вызывает `claude-auto rebase <name>`; **STALE — ВСЕГДА только алерт**,
никогда авто (rebase на несвежей памяти = потеря контекста). Битый
реестр — алерт, не тихое падение.
...
approval `denied`/`executed`/`exec_failed` (а `approved` — только для
НЕисполняемых kind_of: заявку, ждущую диспетчера, ротировать нельзя —
он читает только активный файл; `executing` не ротируется никогда);
incident `resolved`/`wontfix`/`duplicate`; `policy_ack` кроме последнего
на воркера; `agent_run`/`registry_change` — безусловно. `seq` строго
монотонен через ротацию (`rotate-state.json`), порядок crash-безопасный
(архив с дедупом по event_id → rotate-state → атомарный rename активного
файла). Читатели цепочек (`approval-resolve`/`approval-exec`/
`incident-resolve --ref-main`) ищут базу и в архивах.
...
дольше `LIVENESS_HUNG_MIN` (30 мин). Лестница (`decide()`, юнит-тестируется
...
alert-only** (в лестницу не входит, даже под enforce). screenHash в
anim-эпизоде продвигается — если экран после анимации замер, основная
лестница начинает свой отсчёт, anim-эпизод её не глушит.

**`LIVENESS_ENFORCE=1`** — выключатель действий. Без него (дефолт, и на
2026-07-16 остаётся дефолтом) — alert-only: на шагах restart/incident
шлётся разовый `[would-restart]`/`[would-incident]`, но `systemctl
restart` / `incident-open` не вызываются. С флагом — лестница исполняется
...
| Dispatcher state | `~/.claude-control/department/sleep-check-state.json` (+ `dispatcher.lock`, маркеры `exec-stuck-<event_id>`) |
