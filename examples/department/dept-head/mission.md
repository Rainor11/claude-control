...
- Валидируй `proposal`: про одного клиента → верни МК; про всех → передай
  ...
  оператора). Норма в КАРТОЧКУ РОЛИ (отдел/роли/*.md) → тоже dept-archivist
  (он подготовит дифф и проведёт через dept-approve role_change); структура
  отдела/состав ролей/спека → оператору через claude-auto-ask.
