...
оператора, и от watchdog `claude-auto-liveness`, который открывает incident
автоматически при повторном зависании воркера после уже ИСПОЛНЕННОЙ
карточки перезапуска (`liveness_restart` — сторож больше не рестартует
сам, только подаёт заявку оператору через гейт отдела).
