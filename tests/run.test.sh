...
# см. tests/run): механику карантина проверяем на искусственно закарантиненной toy-фикстуре,
# а не на реальном жильце боевого списка (с T6 там tests/dept-exec-runner.test.sh — требует
# живую systemd) — иначе эти сценарии сломались бы в тот день, когда боевой список опять
# опустеет, как он был пуст между T4 и T6. Симлинк-сценарий (В2) карантинит
...
#    на toy-фикстуре, искусственно закарантиненной тест-швом QUARANTINE_TEST_ADD (см. заголовок
#    файла — почему не на боевом жильце списка).
#    toy-quar НАРОЧНО завершился бы PASS (echo+exit 0), если бы запустился, —
