...
test('staleAlertMessage: боевой прогон, доставка провалилась — 🔴-секция про НЕ отправленную просьбу', () => {
  const msg = staleAlertMessage({ dry: false, bus: false, name: 'w1', reason: 'возраст сессии ≥ 14 дн' });
  assert.match(msg, /🔴/);
  assert.match(msg, /НЕ отправлена/);
  // Провал доставки — единственная из трёх веток, где от оператора что-то требуется:
  // в остальных «Что делать» быть не должно, иначе блок обесценивается.
  assert.match(msg, /Что делать/);
});

// Карточка обязана быть валидным HTML: неэкранированное имя воркера или причина увели бы
// всё сообщение в 400 (у telegram_notify.sh фолбэка на ошибку разметки нет).
test('staleAlertMessage: подстановки экранированы, теги сбалансированы', () => {
  const msg = staleAlertMessage({ dry: false, bus: true, name: '<w1>', reason: 'a & b' });
  assert.match(msg, /&lt;w1&gt;/);
