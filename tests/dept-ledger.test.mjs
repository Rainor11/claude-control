...
//
// Замена — deptHome(): выдаёт ПОЛНОЦЕННЫЙ подкорень внутри песочницы раннера (свой sentinel
// и свои заглушки процесс-контроля, см. tests/lib/sandbox.mjs) и возвращает путь его журнала
// <подкорень>/department. Возвращается ПУТЬ, а не объект, ровно чтобы не трогать полсотни
// мест вида readFileSync(join(home, 'events.jsonl')) — они продолжают работать как раньше.
// Соответствие «путь журнала → патч окружения подкорня» держит ROOT_ENV_BY_HOME; envFor()
// собирает из него окружение для запуска CLI.
const ROOT_ENV_BY_HOME = new Map();
const deptHome = (prefix = 'dept-') => {
  const w = makeTestSubroot(prefix);
  ROOT_ENV_BY_HOME.set(w.dept, w.env);
  return w.dept;
};
const envFor = (home, extraEnv = {}) => {
  const rootEnv = ROOT_ENV_BY_HOME.get(home);
  if (!rootEnv) {
    // Не молчаливый фолбэк: неизвестный home означает, что сценарий собрал каталог журнала
    // в обход deptHome() — под маркером такой путь либо снаружи корня (отказ резолвера),
    // либо чужой подкорень. Лучше явная ошибка в тесте, чем загадочный отказ в CLI.
    throw new Error(`envFor: неизвестный DEPT_HOME '${home}' — заводи каталог журнала через deptHome()`);
  }
  return buildEnv(rootEnv, extraEnv);
};
const run = (home, args, input, extraEnv = {}) =>
  execFileSync(CLI, args, { env: envFor(home, extraEnv), input, encoding: 'utf8' });
...
test('policy-current/ack/check — полный цикл соблюдения правил', () => {
  const home = deptHome();
  const pol = mkdtempSync(join(process.env.CLAUDE_CONTROL_TEST_ROOT, 'pol-'));
...
// Codex-крит канона фазы 4, находка 1: policy-ack --actor <чужое-имя> из сессии воркера
// гасил ack-drift датчик за другого — типизированная команда не покрыта PRIVILEGED_KINDS
// (тот гард только на generic append, см. тесты выше про дыру 17.07). Позитивный кейс —
// вызов НЕ из сессии воркера (тесты всегда такие, т.к. execFileSync запускает CLI напрямую,
// а не через claude-бинарь внутри CLAUDE_INSTALL) обязан оставаться свободным. Негативный
// кейс (реальная сессия воркера бьёт --actor чужого) юнит-тестом не проверить без подделки
// /proc — см. R16 и authorizeWithdraw ниже; проверяется static pin (см. блок про К3).
test('policy-ack: --actor "чужого" воркера проходит вне сессии воркера, даже если CLAUDE_AUTO_NAME другой', () => {
  const home = deptHome();
  const pol = mkdtempSync(join(process.env.CLAUDE_CONTROL_TEST_ROOT, 'pol-'));
  writeFileSync(join(pol, 'policy-v1.md'), '# v1\n');
  // CLAUDE_AUTO_NAME=mk-a выставлен в env процесса, но callerWorkerName() смотрит /proc-предков
  // на exe внутри CLAUDE_INSTALL — тестовый процесс (node) туда не попадает, поэтому реальный
  // caller всегда null и anti-spoof не должен сработать даже при несовпадении с --actor.
  const env = { DEPT_POLICY_DIR: pol, CLAUDE_AUTO_NAME: 'mk-a' };
  const r = JSON.parse(run(home, ['policy-ack', '--version', 'v1', '--actor', 'mk-b'], undefined, env));
...
// Codex-крит канона фазы 4 (findings 1-2) — продолжение К3: типизированные policy-ack и
// send обходили PRIVILEGED_KINDS/§5, стоявшие на generic append/адресате. Негативные кейсы
// (реальная сессия воркера) непроверяемы юнитами без подделки /proc (R16) — пин факта, что
// гард вызван, идёт по образцу assertNotWorkerSession-пинов выше.
test('policy-ack зовёт callerWorkerName (anti-spoof чужого --actor)', () => {
  const src = readFileSync(CLI, 'utf8');
...
test('send зовёт callerWorkerName для policy_refresh (only Руководитель + anti-spoof)', () => {
  const src = readFileSync(CLI, 'utf8');
  const fn = src.slice(src.indexOf('function cmdSend'), src.indexOf('function cmdAck'));
  assert.match(fn, /callerWorkerName/,
    'cmdSend обязан звать callerWorkerName для policy_refresh — иначе любой воркер шлёт ложную рассылку (Codex-крит 2, канон фазы 4)');
});
...
// Codex-крит канона фазы 4, находка 2: любой воркер мог разослать policy_refresh, в т.ч.
// подделав --actor dept-head — canon (policy-v9 п. 3.1/8.5) разрешает только исполнителю
// планёрки (systemd-раннер dept-planerka-exec) и Руководителю. Раннер НЕ сессия воркера
// (запускается как отдельный systemd-юнит, не внутри CLAUDE_INSTALL) — R2 требует, чтобы
// его data.from=dept-head продолжал работать без правок. Негативный кейс (реальная сессия
// воркера с ролью ≠ руководитель шлёт policy_refresh, либо бьёт --actor чужого) юнит-тестом
// не проверить без подделки /proc — см. R16; проверяется static pin ниже.
test('send policy_refresh: --actor dept-head проходит вне сессии воркера, даже если CLAUDE_AUTO_NAME другой воркер (раннер планёрки)', () => {
  const home = deptHome();
  run(home, ['registry-set', 'mk-a', '--role', 'мк', '--client', 'a']);
  run(home, ['registry-set', 'dept-head', '--role', 'руководитель']);
  // CLAUDE_AUTO_NAME=mk-a в env, но callerWorkerName() смотрит /proc-предков на exe внутри
  // CLAUDE_INSTALL — тестовый процесс туда не попадает, реальный caller всегда null.
  const env = { CLAUDE_AUTO_NAME: 'mk-a' };
  const r = JSON.parse(run(home, ['send', '--type', 'policy_refresh', '--to', 'mk-a',
    '--subject', 'Планёрка: перечитай policy-v9', '--actor', 'dept-head'], undefined, env));
  assert.ok(r.event_id, 'раннер планёрки (не сессия воркера) обязан слать policy_refresh с --actor dept-head');
});

// /bug ф4 (паритет с N1): гард policy_refresh сравнивал только сырой --actor с реальным
// вызывающим, а identity, которая реально уходит в data.from, резолвится как
// a.actor||CLAUDE_AUTO_NAME||'operator' — из сессии руководителя env-спуф
// `CLAUDE_AUTO_NAME=<чужое-имя> send --type policy_refresh` (без --actor) подделывал
// отправителя рассылки молча. Позитивный кейс: вне сессии воркера (realCaller null)
// резолв from из CLAUDE_AUTO_NAME обязан оставаться свободным и попадать в data.from.
test('send policy_refresh: без --actor, from из CLAUDE_AUTO_NAME — проходит вне сессии воркера', () => {
  const home = deptHome();
  run(home, ['registry-set', 'mk-a', '--role', 'мк', '--client', 'a']);
  run(home, ['registry-set', 'dept-head', '--role', 'руководитель']);
  const env = { CLAUDE_AUTO_NAME: 'dept-head' };
  const r = JSON.parse(run(home, ['send', '--type', 'policy_refresh', '--to', 'mk-a',
    '--subject', 'Планёрка: перечитай policy-v9'], undefined, env));
  const row = JSON.parse(run(home, ['list', '--kind', 'message', '--event-id', r.event_id]));
  assert.equal(row.data.from, 'dept-head', 'from обязан резолвиться из CLAUDE_AUTO_NAME и уходить в data.from');
});

// Пин двух фактов (образец пина N1 у policy-ack): (а) гард policy_refresh сравнивает
// РЕЗОЛВНУТУЮ identity from с realCaller, а не сырой a.actor; (б) from резолвится ДО гарда.
test('send policy_refresh гардит РЕЗОЛВНУТУЮ identity (from), не сырой --actor', () => {
  const src = readFileSync(CLI, 'utf8');
  const fn = src.slice(src.indexOf('function cmdSend'), src.indexOf('function cmdAck'));
  assert.match(fn, /from\s*!==\s*realCaller/,
    'guard обязан сравнивать резолвнутую identity from с realCaller — иначе CLAUDE_AUTO_NAME=<чужое-имя> без --actor подделывает отправителя рассылки (/bug ф4, паритет N1)');
  assert.doesNotMatch(fn, /a\.actor\s*&&\s*a\.actor\s*!==\s*realCaller/,
    'сравнение сырого a.actor вместо резолвнутого from — регресс дыры env-спуфа');
  const fromResolveIdx = fn.search(/const from\s*=\s*a\.actor\s*\|\|\s*process\.env\.CLAUDE_AUTO_NAME/);
  const guardIdx = fn.search(/from\s*!==\s*realCaller/);
  assert.ok(fromResolveIdx >= 0 && guardIdx >= 0 && fromResolveIdx < guardIdx,
    'from обязан резолвиться ДО гарда');
});

// Задача 20.07: identity-гард send() расширен с policy_refresh на ВСЕ типы шины — воркер мог
// отправить обычное question/proposal/handoff/incident/kb_change_request/decision_request с
// --actor <чужое-имя> (или CLAUDE_AUTO_NAME=<чужое-имя> без --actor), и это же попутно обходило
// МК→МК бан (он читает roleOf(from), а from при спуфинге — заявленное, не настоящее имя).
// Негативный кейс (реальная сессия воркера с чужим --actor/CLAUDE_AUTO_NAME) непроверяем без
// подделки /proc (R16, как и у policy-ack/policy_refresh/approval-open выше) — только static pin.
test('send: --actor "кто-то" для обычного типа (question) вне сессии воркера — по-прежнему проходит (identity-гард расширен на все типы, НЕ-воркера не касается)', () => {
  const home = deptHome();
  run(home, ['registry-set', 'dept-head', '--role', 'руководитель']);
  const r = JSON.parse(run(home, ['send', '--type', 'question', '--to', 'dept-head',
    '--subject', 'x', '--body', 'y', '--actor', 'mk-a']));
  const row = JSON.parse(run(home, ['list', '--kind', 'message', '--event-id', r.event_id]));
  assert.equal(row.data.from, 'mk-a', 'вне сессии воркера --actor обязан свободно резолвиться в data.from для ЛЮБОГО типа — не только policy_refresh');
});

test('send: identity-гард (from !== realCaller) применяется КО ВСЕМ типам, не только policy_refresh (регресс-пин)', () => {
  const src = readFileSync(CLI, 'utf8');
  const fn = src.slice(src.indexOf('function cmdSend'), src.indexOf('function cmdAck'));
  assert.doesNotMatch(fn, /if\s*\(a\.type\s*===\s*'policy_refresh'\)\s*\{\s*(?:\/\/[^\n]*\n\s*)*const realCaller/,
    'гард НЕ должен быть вложен внутрь if(a.type===policy_refresh) — иначе обычные типы (question/proposal/handoff/incident/kb_change_request/decision_request) снова спуфятся мимо гарда (регресс задачи 20.07)');
  const guardIdx = fn.search(/from\s*!==\s*realCaller/);
  const typeCheckIdx = fn.search(/a\.type\s*===\s*'policy_refresh'/);
  assert.ok(guardIdx >= 0 && typeCheckIdx >= 0 && guardIdx < typeCheckIdx,
    'identity-гард обязан идти РАНЬШЕ ветки policy_refresh — он общий для всех типов шины, не эксклюзивный довесок к рассылке');
});

test('send: ролевой гард (руководитель-only) остаётся эксклюзивным для policy_refresh, не расползся на все типы', () => {
  const src = readFileSync(CLI, 'utf8');
  const fn = src.slice(src.indexOf('function cmdSend'), src.indexOf('function cmdAck'));
  assert.match(fn, /a\.type\s*===\s*'policy_refresh'\s*&&\s*realCaller\s*&&\s*roleOf\(realCaller\)\s*!==\s*'руководитель'/,
    'ролевая проверка (только Руководитель может слать) обязана остаться ПОД условием a.type===policy_refresh — иначе МК не сможет отправить обычный question/proposal из своей сессии');
});

// Codex second opinion (сессия сторожа-кнопки, liveness_restart, T4 F1): воркер мог подать
// approval-open от чужого имени (`--actor watchdog` / `--actor dept-head`) и вместе с уже
// разрешённым воркеру claude-auto-request --action dept-approval предъявить оператору
// правдоподобную карточку, целящую в ДРУГОГО воркера. Гард — тот же приём, что у
// policy-ack/send(policy_refresh) выше. Позитивный кейс (как у соседей): вызов НЕ из сессии
// воркера (execFileSync — не claude-бинарь внутри CLAUDE_INSTALL) обязан оставаться свободным —
// realCaller всегда null. Штатные писатели: dept-approve (worker-only) резолвит actor тем же
// /proc-приёмом и передаёт --actor <себя>, поэтому from===realCaller и гард пропускает —
// негативный кейс (реальная сессия воркера с чужим --actor) непроверяем без подделки /proc
// (R16), проверяется static pin ниже.
test('approval-open: --actor "чужого" воркера проходит вне сессии воркера, даже если CLAUDE_AUTO_NAME другой', () => {
  const home = deptHome();
  const env = { CLAUDE_AUTO_NAME: 'mk-a' };
  const r = JSON.parse(run(home, ['approval-open', '--kind-of', 'liveness_restart', '--summary', 's',
    '--actor', 'watchdog'], undefined, env));
  assert.ok(r.event_id, 'вне сессии воркера approval-open от имени другого (--actor) обязан проходить');
  const row = JSON.parse(run(home, ['list', '--kind', 'approval', '--event-id', r.event_id]));
  assert.equal(row.data.from, 'watchdog');
});

test('approval-open: без --actor, только CLAUDE_AUTO_NAME=<кто-то> — проходит вне сессии воркера', () => {
  const home = deptHome();
  const env = { CLAUDE_AUTO_NAME: 'mk-c' };
  const r = JSON.parse(run(home, ['approval-open', '--kind-of', 'other', '--summary', 's'], undefined, env));
  const row = JSON.parse(run(home, ['list', '--kind', 'approval', '--event-id', r.event_id]));
  assert.equal(row.data.from, 'mk-c', 'from обязан резолвиться из CLAUDE_AUTO_NAME и уходить в data.from');
});

// Пин двух фактов (образец пина у policy-ack/send выше): (а) гард сравнивает РЕЗОЛВНУТУЮ
// identity from с realCaller, а не сырой a.actor; (б) from резолвится ДО гарда.
test('approval-open гардит РЕЗОЛВНУТУЮ identity (from), не сырой --actor (static pin)', () => {
  const src = readFileSync(CLI, 'utf8');
  const fn = src.slice(src.indexOf('function cmdApprovalOpen'), src.indexOf('function cmdApprovalResolve'));
  assert.match(fn, /callerWorkerName/,
    'cmdApprovalOpen обязан звать callerWorkerName — иначе воркер подаёт заявку от чужого имени (Codex second opinion, T4 F1)');
  assert.match(fn, /from\s*!==\s*realCaller/,
    'guard обязан сравнивать резолвнутую identity from с realCaller, а не a.actor — иначе CLAUDE_AUTO_NAME=<чужое-имя> без --actor подделывает заявителя молча');
  assert.doesNotMatch(fn, /a\.actor\s*&&\s*a\.actor\s*!==\s*realCaller/,
    'сравнение сырого a.actor вместо резолвнутого from — регресс дыры env-спуфа (по образцу N1/паритета ф4)');
  const fromResolveIdx = fn.search(/const from\s*=\s*a\.actor\s*\|\|\s*process\.env\.CLAUDE_AUTO_NAME/);
  const guardIdx = fn.search(/from\s*!==\s*realCaller/);
  assert.ok(fromResolveIdx >= 0 && guardIdx >= 0 && fromResolveIdx < guardIdx,
    'from обязан резолвиться ДО гарда, иначе сравнение идёт со старым/неопределённым значением');
});
...
// --- M4 (Codex-аудит): resolve/withdraw читали effectiveStatus ТОЛЬКО по активному файлу —
// полностью отротированная закрытая цепочка (approval_status тоже уехал в архив) читалась бы
// заново как 'open'. cmdApprovalExec уже был архивно-осведомлён (readChainStatuses) — resolve
// теперь читает через тот же approvalEffectiveStatus. Оба теста ниже падали ДО фикса: ни один
// из "ремней" (второй ремень withdrawn, гард "уже исполнен") не срабатывал на архиве. ---

test('M4: approval-resolve поверх ЗААРХИВИРОВАННОЙ withdrawn-цепочки — второй ремень видит архив', () => {
  const home = deptHome();
  const a = JSON.parse(run(home, ['approval-open', '--kind-of', 'outgoing', '--summary', 'x', '--actor', 'mk-a']));
  run(home, ['append', '--kind', 'approval_status', '--data',
    JSON.stringify({ ref: a.event_id, status: 'withdrawn' }), '--actor', 'mk-a']);
  ...
  writeFileSync(led, readFileSync(led, 'utf8').split('\n').filter(Boolean)
    .map((l) => JSON.stringify({ ...JSON.parse(l), ts: old })).join('\n') + '\n');
  const r = JSON.parse(run(home, ['rotate', '--days', '30']));
  assert.ok(r.rotated >= 2, 'цепочка обязана уехать в архив целиком — иначе тест не проверяет архивный путь');
  // до фикса: eff='open' заново (активный файл пуст после rotate) → approve поверх отозванной
  // прошёл бы, второй ремень против гонки withdraw↔✅ не увидел бы архивный withdrawn.
  assert.throws(() => run(home, ['approval-resolve', a.event_id, '--status', 'approved', '--actor', 'operator']),
    /отозван/i);
});

test('M4: approval-resolve поверх ЗААРХИВИРОВАННОЙ executed-цепочки — «уже исполнен» видит архив', () => {
  const home = deptHome();
  const a = JSON.parse(run(home, ['approval-open', '--kind-of', 'outgoing', '--summary', 'x', '--actor', 'mk-a']));
  run(home, ['approval-resolve', a.event_id, '--status', 'approved', '--actor', 'operator']);
  run(home, ['approval-exec', a.event_id, '--status', 'executed', '--actor', 'dispatcher']);
  const led = join(home, 'events.jsonl');
  const old = new Date(Date.now() - 40 * 86400_000).toISOString();
  writeFileSync(led, readFileSync(led, 'utf8').split('\n').filter(Boolean)
    .map((l) => JSON.stringify({ ...JSON.parse(l), ts: old })).join('\n') + '\n');
  const r = JSON.parse(run(home, ['rotate', '--days', '30']));
  assert.ok(r.rotated >= 3, 'цепочка approval+approved+executed обязана уехать в архив целиком');
  // до фикса: eff='open' заново → resolve приняло бы новое решение поверх уже исполненной заявки.
  assert.throws(() => run(home, ['approval-resolve', a.event_id, '--status', 'denied', '--actor', 'operator']),
    /исполнен/i);
});
