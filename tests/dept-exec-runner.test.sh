...
# Структура песочницы ЗЕРКАЛИТ репо: скрипты в SANDBOX/bin/, библиотеки в SANDBOX/lib/.
# Иначе node-скрипты (dept-ledger/dispatcher/exec-runner) не найдут свои `require('../lib/…')`
# и `. ../lib/notify-card.sh` — при плоском SANDBOX `../lib` резолвится в /tmp/lib (нет такого).
# Копируем ВЕСЬ lib/, а не выборочно: runtime-root.js тянет за собой process-control.js и др.
SANDBOX="$(mktemp -d)"
mkdir -p "$SANDBOX/bin"
cp "$DIR/bin/dept-ledger" "$DIR/bin/dept-dispatcher" "$DIR/bin/dept-exec-runner" "$DIR/bin/dept-memory-freshness" "$SANDBOX/bin/"
cp -r "$DIR/lib" "$SANDBOX/lib"
cat > "$SANDBOX/bin/dept-sleep-exec" <<'EOF'
#!/bin/bash
...
TELEGRAM_NOTIFY="$SANDBOX/bin/fake-notify.sh"
...
DL="$SANDBOX/bin/dept-ledger"
DISPATCHER="$SANDBOX/bin/dept-dispatcher"
...
# Контекст теперь разложен по ПОЛЯМ карточки, а не слеплен в одну строку: проверяем, что
# каждое поле на месте (summary / автор / event_id), а не точную фразу — формулировка
# заголовка косметическая, а вот пропажа автора или ID делает алерт нечитаемым без ledger.
human_ok=""
for _ in $(seq 1 10); do
  if grep -q "Заявка исполнена" "$NOTIFY_LOG" 2>/dev/null \
     && grep -q "смок sleep" "$NOTIFY_LOG" 2>/dev/null \
     && grep -q "dept-head" "$NOTIFY_LOG" 2>/dev/null \
     && grep -q "$eid" "$NOTIFY_LOG" 2>/dev/null; then human_ok=1; break; fi
  sleep 0.5
done
[ -n "$human_ok" ] || fail "✅-алерт раннера без человеческого контекста (summary/from/id): $(cat "$NOTIFY_LOG" 2>/dev/null)"
# Карточка обязана уходить размеченной и с `--` перед текстом (иначе сообщение с ведущим
# '-' было бы перечитано нотификатором как флаг).
grep -q -- '--parse-mode' "$NOTIFY_LOG" || fail "✅-алерт ушёл без --parse-mode: $(cat "$NOTIFY_LOG")"
...
# Фидбэк оператора 16.07: recovery-алерт тоже очеловечен — summary + from + event_id. Теперь
# они разложены по полям карточки, а не слеплены в одну строку, поэтому проверяем наличие
# каждого, а не точную фразу.
if ! grep -q "смок stuck" "$NOTIFY_LOG" || ! grep -q "dept-head" "$NOTIFY_LOG" || ! grep -q "$eid2" "$NOTIFY_LOG"; then
  fail "recovery-алерт без человеческого контекста (summary/from/id): $(cat "$NOTIFY_LOG")"
fi
...
FAKE_EXEC_SLEEP=0 DEPT_EXEC_RUNNER_TIMEOUT_MS="15min" \
  "$SANDBOX/bin/dept-exec-runner" --approval "$eid3" --executor "$SANDBOX/bin/dept-sleep-exec" >/dev/null 2>&1 || true
...
FAKE_EXEC_SLEEP=0 DEPT_EXEC_RUNNER_TIMEOUT_MS=5000 \
  "$SANDBOX/bin/dept-exec-runner" --approval "$eid4" --executor "$SANDBOX/bin/dept-sleep-exec" >/dev/null 2>&1 || true
...
mv "$SANDBOX/bin/dept-exec-runner" "$SANDBOX/bin/dept-exec-runner.bak" # раннера больше нет по ожидаемому пути
...
grep -qi "раннер НЕ запустился" "$NOTIFY_LOG" || fail "нет алерта о незапустившемся раннере: $(cat "$NOTIFY_LOG")"
mv "$SANDBOX/bin/dept-exec-runner.bak" "$SANDBOX/bin/dept-exec-runner" # вернуть на место
