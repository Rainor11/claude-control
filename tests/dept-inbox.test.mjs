...
// Багхант 16.07: approved-исполняемая заявка НЕ от руководителя/operator — pickExecutable
// диспетчера её не возьмёт никогда; фаза 'approved_foreign' честно говорит об этом
// вместо вечного «ждёт диспетчера».
test('renderApprovalsPage: фаза approved_foreign — «диспетчер НЕ возьмёт», не «ждёт диспетчера»', () => {
  const html = renderApprovalsPage({ approvals: [], incidents: [], recent: [], executed: [],
    executingNow: [{ event_id: 'evt_7_frgn', ts: new Date().toISOString(), phase: 'approved_foreign',
      data: { kind_of: 'worker_spawn', from: 'mk-prodmash', summary: 'найм от не-руководителя' } }],
    registry: { workers: {} }, policy: {} });
  assert.ok(html.includes('диспетчер НЕ возьмёт'));
  assert.ok(!html.includes('ждёт диспетчера'));
});

// Пин 5-й копии списка exec-kinds (M-1 запинил 4: EXEC_KINDS dispatcher, EXECUTORS,
// WHITELIST dept-exec-runner, EXEC_KINDS_ROT ledger; эта — EXEC_KINDS_INBOX в dept-inbox).
// Разъезд = секция «Исполняются» перестаёт видеть approved-заявки нового kind_of.
test('EXEC_KINDS_INBOX (dept-inbox) синхронен с EXEC_KINDS dispatcher', () => {
  const src = readFileSync(new URL('../bin/dept-inbox', import.meta.url), 'utf8');
