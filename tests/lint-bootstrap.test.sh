...
#
# LEGACY_ALLOWLIST ниже — ЯВНЫЙ список файлов, для которых отсутствие bootstrap ОЖИДАЕМО (не
# ошибка). Список ОБЯЗАН только СОКРАЩАТЬСЯ: если файл из allow-list ВДРУГ обзавёлся bootstrap
# (мигрировали, но забыли вычеркнуть отсюда) — это ТОЖЕ падение лита (см. проверку №2 ниже),
# а не тихий проход. Так "заберут — не забудут вычеркнуть" ловится механически, а не на
# честном слове.
#
# СОСТОЯНИЕ ПОСЛЕ T6: из 23 записей осталась ОДНА — tests/dept-exec-runner.test.sh, и это
# ОСОЗНАННОЕ исключение, а не недоделанная миграция. Файл закарантинен в tests/run (требует
# ЖИВУЮ systemd user session: dept-dispatcher стартует раннер настоящим `systemd-run --user`,
# под заглушкой смысл e2e исчезает) — то есть через раннер он не запускается ВООБЩЕ, а
# запускается вручную на живом контуре, когда его результат реально нужен. Подключить ему
# пролог значило бы сделать этот единственный оставшийся способ запуска НЕВОЗМОЖНЫМ: пролог
# по определению отказывает при запуске без песочницы раннера. Вычеркнуть запись можно будет
# только вместе со снятием карантина.
#
