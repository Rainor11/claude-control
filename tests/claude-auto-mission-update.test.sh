...
#
# Всё в песочнице раннера; боевой флот/ledger не трогается, tmux не нужен (worker_busy: нет
# tmux-сессии → не busy). Паттерн — tests/claude-auto-spawn.test.sh.
#
# T6: было три собственных корня во временном каталоге (CLAUDE_CONTROL_DIR/DEPT_HOME/HOME =
# "$(mktemp -d)/…") — все СНАРУЖИ тестового корня, резолвер T1 законно отказывал. Стало:
# корень задаёт раннер, тест его не переопределяет, а знает соответствие профилей:
# control_only → сам корень, dept_only → <корень>/department. HOME тоже подставляет раннер
# (свежий пустой каталог в песочнице) — отдельный override больше не нужен, изоляция
# ~/.claude/projects от этого не пострадала.
set -u
...
CC="$CLAUDE_CONTROL_TEST_ROOT"              # профиль control_only под маркером
DEPT="$CLAUDE_CONTROL_TEST_ROOT/department" # профиль dept_only под маркером
...
mkdir -p "$CC/workers/w1/state" "$TMP/cwd" "$TMP/brainpath" "$HOME/.claude/projects/-proj"
printf '{"session_id":"%s","cwd":"%s","seeded":true}\n' "$sid" "$TMP/cwd" > "$CC/workers/w1/spec.json"
printf '{"workers":{"w1":{"state":"active","brain_path":"%s"}}}\n' "$TMP/brainpath" > "$CC/autonomous.json"
...
grep -q 'новая миссия w1 (смок rc=4)' "$CC/workers/w1/CLAUDE.md" \
  || fail "миссия не записана в CLAUDE.md воркера"
grep -q 'новая миссия w1 (смок rc=4)' "$DEPT/missions/w1.md" \
  || fail "аудит-копия миссии не записана в <корень>/department/missions/w1.md"
...
jq -e --arg s "$sid" '.session_id == $s' "$CC/workers/w1/spec.json" >/dev/null \
