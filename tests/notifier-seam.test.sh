...
elif printf '%s' "$got" | command grep -q "No module named 'aiogram'"; then
  # Отсутствие СТОРОННЕЙ зависимости бота — не предмет T8 (на сервере она в bot/venv).
  # Пропуск ТОЧЕЧНЫЙ: любой другой вывод, включая боевой путь нотификатора, — падение.
  echo "ПРОПУСК: 7 — aiogram не установлен (ни bot/venv, ни системный), импорт бота невозможен"
else
  fail "импорт rnr_workers_bot под маркером дал TG_NOTIFY='$got', ожидалась заглушка '$STUB'"
fi
